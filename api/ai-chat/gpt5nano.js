...
async function gpt5nano(prompt, system = "") {
    const chatHistory = [];
...
if (require.main === module) {
    ...
    if (!prompt) { console.log('Usage: node gpt5nano.js "pertanyaan" ["system"]'); process.exit(1); }
    gpt5nano(prompt, system)
        .then(r  => console.log("✅ Response:\n" + (typeof r === "string" ? r : JSON.stringify(r, null, 2))))
        .catch(e => { console.error("❌ Error:", e.message); process.exit(1); });
} else {
    module.exports = {
        name:     "GPT-5 Nano",
        desc:     "Chat dengan GPT-5 Nano via deepai.org — gratis tanpa login.",
        category: "AI CHAT",
        ...
        async run(req, res) {
            try {
                ...
                const result = await gpt5nano(prompt, _system);
