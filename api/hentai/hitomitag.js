...
module.exports = {
    name: "HitomiByTag",
    desc: "Ambil gallery dari Hitomi.moe berdasarkan tag, artist, character, parody, atau group.",
    category: "Hentai",
    ...
    async run(req, res) {
        ...
        const validTypes = ["tag", "artist", "character", "parody", "group", "language", "type"];
        ...
        try {
            const html = await fetchUrl(`https://hitomi.moe/${type}/${slug}/?page=${page}`);
            const galleries = parseGalleries(html);
            const maxPage = parseMaxPage(html);
            res.status(200).json({ status: true, creator: "Xena", type, slug, page: parseInt(page), maxPage, total: galleries.length, result: galleries });
        } catch (e) {
